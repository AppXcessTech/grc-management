...
import { Loader2, Monitor, Laptop, Smartphone, Cpu, Plus, ArrowRight } from 'lucide-react';
import api from '../../services/api';

interface Counts {
  total: number;
  active: number;
  laptops_workstations: number;
  mobile_devices: number;
  specialized_devices: number;
}
...
const EndpointDashboard = () => {
  ...
  useEffect(() => {
    api.get('/api/endpoint-devices/counts')
  ...
  const statCards = [
    { label: 'Total Devices', value: counts?.total ?? 0, icon: Monitor, color: '#3b82f6' },
    { label: 'Active', value: counts?.active ?? 0, icon: Monitor, color: '#22c55e' },
    { label: 'Laptops & Workstations', value: counts?.laptops_workstations ?? 0, icon: Laptop, color: '#8b5cf6' },
    { label: 'Mobile Devices', value: counts?.mobile_devices ?? 0, icon: Smartphone, color: '#f97316' },
    { label: 'Specialized Devices', value: counts?.specialized_devices ?? 0, icon: Cpu, color: '#06b6d4' },
  ];
    ...
    <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <h1>Endpoint Devices Dashboard</h1>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.5rem' }}>
          <button className="btn btn-primary" onClick={() => navigate('/assets/devices/list')}>
