...
const PeopleIntegrations = () => {
  ...
  const handleSetup = () => {
    navigate('/assets/people/integrations/setup');
  };
  ...
  const handleSync = async () => {
    ...
    setError('');
    try {
      const res = await api.post('/api/integrations/okta/sync');
      setResult(res.data);
    } catch (err: any) {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem', fontSize: '0.875rem' }}>
        Connect Okta to sync user data. Click <strong>Configure</strong> to enter your domain and API token, then <strong>Sync Now</strong> to pull users.
      </p>

      <div className="card" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
        <div style={{ width: 48, height: 48, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#007dc115', color: '#007dc1', fontSize: 24 }}>
          <i className="ti ti-brand-okta"></i>
        </div>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: '0.9375rem', fontWeight: 600, margin: 0 }}>Okta</h3>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', margin: '2px 0 0' }}>
            Click <strong>Configure</strong> to enter your domain and token, then <strong>Sync Now</strong>.
          </p>
        </div>
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '0.75rem' }}>
              <div style={{ padding: '0.75rem', borderRadius: 8, background: 'var(--background)', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Total from Okta</div>
              </div>
            ...
            {result.created > 0 && (
              <p style={{ color: '#059669', fontSize: '0.8125rem', marginTop: '0.5rem' }}>
                <i className="ti ti-circle-check"></i> Successfully created {result.created} users from Okta.
              </p>
            )}
          </div>
        </div>
...
export default PeopleIntegrations;
